...
import { apiGetAccounts, apiWithdraw } from '../utils/api';
...
export default function Withdraw() {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      const { balance, transaction } = await apiWithdraw(accountNo, amount);
      setSuccess(`Withdrawal successful! New balance: Rs. ${balance.toFixed(2)}`);
  ...
  if (accounts.length === 0) {
    return (
      <div className="max-w-md mx-auto px-4 py-10">
        <Alert type="info">You need an account before you can withdraw. Create one from the dashboard.</Alert>
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-10">
      <Card>
        <h1 className="text-xl font-display font-bold text-ink-800 mb-1">Withdraw Money</h1>
        <p className="text-ink-400 text-sm mb-6">Withdraw funds from one of your accounts.</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Input
            ...
            placeholder="500"
          />
          <Button type="submit" variant="danger" disabled={submitting} className="w-full mt-2">
